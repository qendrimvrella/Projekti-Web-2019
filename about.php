<?php
    include 'header.php';
?> 
</div>
</div>

    <div id="about">
        <div class="container">
            <div class="main-col">
                <h1 class="marg-tb-15"><span class="purple-fonts">About</span> Us</h1>
                <p>Mr do raising article general norland my hastily. Its companions say uncommonly pianoforte favourable. Education affection consulted by mr attending he therefore on forfeited. High way more far feet kind evil play led. Sometimes furnished collected add for resources attention. Norland an by minuter enquire it general on towards forming. Adapted mrs totally company two yet conduct men do offending supported extremity breakfast by listening. Decisively advantages nor expression unpleasing she led met. Estate was tended ten boy nearer seemed. As so seeing latter he should thirty whence. Steepest speaking up attended it as. Made neat an on be gave show snug tore.</p>
				<p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Est sit expedita omnis distinctio quos doloremque temporibus officiis quas? Excepturi labore unde, amet laborum perspiciatis veniam nam deserunt laudantium eos impedit exercitationem sequi? Nam, dolorum ea explicabo rerum dolor omnis modi.
				</p>
				<p>Sportsman do offending supported extremity breakfast by listening. Decisively advantages nor expression unpleasing she led met. Estate was tended ten boy nearer seemed. As so seeing latter he should thirty whence. Steepest speaking up attended it as. Made neat an on be gave show snug tore. </p>
            </div>
            <div class="sidebar">
                <h3 class="marg-tb-15">What We Do</h3>
                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Praesentium libero culpa, excepturi cupiditate sed corrupti cum fugit exercitationem hic omnis itaque corporis nemo doloremque quod harum repudiandae quibusdam qui a fuga. Assumenda quod hic in ea neque quasi itaque inventore!</p>
            </div>
        </div>
    </div>

    <div id="clients">
        <div class="container">
            <h2>Clients We Have Work With</h2>
            <div class="box-client">
                <img src="./sources/client-1.png" alt="photo">
                <img src="./sources/client-2.png" alt="photo">
                <img src="./sources/client-3.png" alt="photo">
                <img src="./sources/client-4.png" alt="photo">
                <img src="./sources/client-5.png" alt="photo">
            </div>
        </div>
    </div>

<?php
    include 'footer.php';
?>